...
export async function POST(req:NextRequest){
  ...
  await connectMongoDb();
    ...
    try {
        ...
        if(customer){
            return NextResponse.json({message:"Account Exists with this email id",success:false},{status:400});
        }
    ...
    const emailResponse = await sendVerificationMail(
      ...
      if (!emailResponse.success) {
        ...
        return NextResponse.json({message:"Account Created, OTP shared to email. Please verify!",success:true},{status:201})


        
    } catch (error) {
        return NextResponse.json({message:"Customer Registration Error:",error},{status:500});
    }
}
